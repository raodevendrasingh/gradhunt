...
export const useCompanySlugCheck = () => {
	...
	const checkSlug = useDebounceCallback(async (companySlug: string) => {
		if (companySlug.length > 4 && isValidSlug(companySlug)) {
			setIsCheckingSlug(true);
			setSlugMsg("");
			try {
				const url = `${apiUrl}/api/users/check-company-slug/?companySlug=${companySlug}`;
				const response = await axios.get(url);
				if (response.data.exists) {
					setSlugMsg(
						"This URL is already taken, choose something else!"
					);
